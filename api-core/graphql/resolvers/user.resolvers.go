...
/*
	...
	getUser, err := client.User.Query().Where(user.UsernameEQ(*input.Username)).Only(ctx)
	if err != nil {
	...
	if input.NewPassword != nil && *input.NewPassword != "" {
		...
		if claims.Username == *input.Username || claims.Role != "Admin" {
			// Mevcut şifreyi kontrol et
			err = bcrypt.CompareHashAndPassword([]byte(getUser.Password), []byte(*input.Password))
			if err != nil {
				return nil, fmt.Errorf("mevcut şifre yanlış")
			}
		}
