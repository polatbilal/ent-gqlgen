...
var companyownerImplementors = []string{"CompanyOwner", "Node"}

// IsNode implements the Node interface check for GQLGen.
func (*CompanyOwner) IsNode() {}
...
func (c *Client) noder(ctx context.Context, table string, id int) (Noder, error) {
	...
	case companyengineer.Table:
		...
		return query.Only(ctx)
	case companyowner.Table:
		query := c.CompanyOwner.Query().
			Where(companyowner.ID(id))
		if fc := graphql.GetFieldContext(ctx); fc != nil {
			if err := query.collectField(ctx, true, graphql.GetOperationContext(ctx), fc.Field, nil, companyownerImplementors...); err != nil {
				return nil, err
			}
		}
		return query.Only(ctx)
...
func (c *Client) noders(ctx context.Context, table string, ids []int) ([]Noder, error) {
	...
	case companyowner.Table:
		query := c.CompanyOwner.Query().
			Where(companyowner.IDIn(ids...))
		query, err := query.CollectFields(ctx, companyownerImplementors...)
		if err != nil {
			return nil, err
		}
		nodes, err := query.All(ctx)
