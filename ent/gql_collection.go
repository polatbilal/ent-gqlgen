...
func (_q *JobPaymentsQuery) collectField(ctx context.Context, oneNode bool, opCtx *graphql.OperationContext, collected graphql.CollectedField, path []string, satisfies ...string) error {
	...
	for _, field := range graphql.CollectFields(opCtx, collected.Selections, satisfies) {
		...
		case "invoiceamount":
			if _, ok := fieldSeen[jobpayments.FieldInvoiceAmount]; !ok {
				selectedFields = append(selectedFields, jobpayments.FieldInvoiceAmount)
				fieldSeen[jobpayments.FieldInvoiceAmount] = struct{}{}
			}
		case "createdat":
